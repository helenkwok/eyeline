...
2. **Pillar 2: Multimodal Continuity Adjudication (Google ADK & Gemini 3.8 Flash on Vertex AI)**
...
- **Negative Control Evaluation**:
  ...
  - Report empirical False-Positive Rate (FPR = FP / C) and catalog tripped controls. Negative controls measure discriminative power and prevent gaming metrics by indiscriminately flagging deltas.
...
- **Standing Instruction — State Synchronization (`docs/STATE.md`)**:
  ...
  - An OOM or session interruption must cost only one file read (`docs/STATE.md`) instead of a multi-turn deep dive.
  - On a fresh clone, `docs/STATE.md` serves as the immediate "where is this project" summary that judges read first.
